...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(Int, Iterable[(Posting, Posting)])] = {
    ...
    val filteredAnswers = postings filter { p => p.postingType == 2 }
    val answers = for {
      posting <- filteredAnswers
  ...
  def scoredPostings(grouped: RDD[(Int, Iterable[(Posting, Posting)])]): RDD[(Posting, Int)] = {
    ...
    for {
      g <- grouped
    } yield (g._2.head._1, answerHighScore(g._2.map(_._2).toArray))
  }
  ...
  def vectorPostings(scored: RDD[(Posting, Int)]): RDD[(Int, Int)] = {
    ...
    // for each score, if index not none, yield index, score.score
    (for {
      s <- scored
      option = firstLangInTag(s._1.tags, langs)
    } yield (option.getOrElse(0)*langSpread, s._2)).persist()
  }
